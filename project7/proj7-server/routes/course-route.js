...
router.get("/", (req, res) => {
  // course跟user連接(instructor),
  // populate 會一起把instructor資料丟上來course.find, 他就是接到Usermodel的內容，關聯好用的地方
  Course.find({})
...
router.get("/instructor/:_insturctor_id", (req, res) => {
  let { _instructor_id } = req.params;
  Course.find({ insturctor: _instructor_id })
    .populate("instructor", ["username, email"])
    .then((data) => {
      res.send(data);
    })
...
// query all courses from sutdent_id
router.get("/student/:_student_id", (req, res) => {
  let { _student_id } = req.params;

  //find if students contains student_id
...
router.patch("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the insturctor of this course ow web admin can edit this course",
    });
  }
});
...
router.delete("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of the course or web admin can edit the course",
    });
  }
});
